...
import {
    getMultiMenu, getMultiMenuGetFile,
    selectMultiMenu
} from "../multimenu/MultiMenuReducer";
...
function Product() {
    ...
    const multiMenuResult = useSelector(state => state.multiMenu.result);
    const multiMenu = useSelector(state => state.multiMenu.multiMenuProduct);
                        ...
                        <Col xs={12} sm={12} md={12} lg={12} xl={12} key={index} className={"shadow p-2 mt-2"} style={{backgroundColor:'white'}}>
                            <div className={"d-flex justify-content-between p-2"}>
                                <div className={"d-flex "}>
                                    <Button variant={"outline-danger"} size={"sm"} className={"m-1"} onClick={()=>dispatch(getMultiMenuGetFile(item?.id))}>
                                        Yuklash
                                    </Button>
